...
function resample() {
	...
	d3.select("#plot4").select("#alpha_circle").transition().duration(1000)
	.attr("cx", "" + xl(alpha_sample))
	.attr("cy", "" + yl(alpha_sample_density));

	d3.select("#plot4").select("#alpha_vline").transition().duration(1000)
		.attr("x1", xl(alpha_sample))
		.attr("y1", "" + (height-marginBottom))
		.attr("x2", xl(alpha_sample))
		.attr("y2", yl(alpha_sample_density));
	
	d3.select("#plot4").select("#beta_circle").transition().duration(1000)
		.attr("cx", "" + xr(beta_sample))
		.attr("cy", "" + yr(beta_sample_density));
	
	d3.select("#plot4").select("#beta_vline").transition().duration(1000)
		.attr("x1", xr(beta_sample))
		.attr("y1", "" + (height-marginBottom))
		.attr("x2", xr(beta_sample))
		.attr("y2", yr(beta_sample_density));
	...
	d3.select("#plot5").select("#sigmoid").transition().duration(1000)
...
svg.append("g")
  ...
  svg.append("path")
	...
	// legend
    // .append('text')
    // .text("α:" + alpha_sample + "\u00A0 \u00A0 \u00A0 \u00A0" +  "β:" + beta_sample)
	// .attr("id", "legend1")
	// .style('font-family', 'serif')
...
var div = document.querySelector("#plot4");
div.append(charts());

var div = document.querySelector("#plot5");
let new_sigmoid = sigmoid(alpha_sample, beta_sample);
div.append(chart(new_sigmoid, challenger_data));

d3.select('#resample_button').on("click", function() {
	resample();
})
d3.select('#resample_button2').on("click", resample)